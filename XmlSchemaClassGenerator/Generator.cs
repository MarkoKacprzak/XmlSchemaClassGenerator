...
namespace XmlSchemaClassGenerator
{
    public class Generator
    {
        ...
        public void Generate(IEnumerable<string> files)
        {
            ...
            var namespacesWithPartial = GenerateCode();            
            foreach (var namespaces in namespacesWithPartial)
            {
                GenerateToFile(namespaces.MainPartial, false);
                namespaces.AdditionalPartial.ForEach(a => GenerateToFile(a, true));
                if (_configuration.ValueTypeEnable)
                    GenerateToFile(ValueObjectNamespace.GettNamespace(_configuration), false, true);
            }
        }

        private void GenerateToFile(CodeNamespace ns,bool isPartial,bool isClassHelper = false)
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                {
                    path = Path.Combine(OutputFolder, ns.Name + (isPartial ? ".partial" : "")  + ".cs");
                }
                Log?.Invoke(path); File.WriteAllText(path, s);
            }
        }

        private IEnumerable<CodeNamespaceWihPartial> GenerateCode()
        {
